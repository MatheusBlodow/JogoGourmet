		...
		for(int j=0; j<grupo.size();j++) {
			...
			if(validorEscolha == 1 && j == grupo.size()-1) {
				...
				if(validorEscolha == 0) {
					...
					tipoPrato = JOptionPane.showInputDialog(null,"qual prato você pensou?");
					prato.add(Integer.toString(grupo.size()) + " "+ tipoPrato);
					tipoGrupo = JOptionPane.showInputDialog(null,"é ________ mas Bolo de Chocolate não");
					grupo.add(tipoGrupo);
					break;
				}	
